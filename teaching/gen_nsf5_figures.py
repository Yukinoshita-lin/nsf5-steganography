...
def solve_dry_two(H, dry1, d):
    """在 1-indexed 干列里找 单列 或 双列异或 等于 d 的组合。返回 1-indexed 列列表。"""
    for c in dry1:
...
# ----------------------------------------------------------------------
# 1) 湿点/干点 划分 + xv = 像素-128
# ----------------------------------------------------------------------
def _draw_h(ax, H, wet, dry, sol=None):
    p, n = H.shape
    bg = np.zeros((p, n))
    for c in range(n):
        bg[:, c] = 0.55 if (c + 1) in wet else 0.0   # 湿列淡红, 干列正常
...
def fig_wet_dry():
    ...
    ax.set_ylabel("|xv| = |像素−128|", fontsize=9)
    for i, w in enumerate(wet):
        ax.text(i, np.abs(xv)[i] + 0.6, "湿" if w else "干", ha="center", fontsize=7.5,
                color=WARM if w else GREEN)
    ax.axhline(1.0, ls="--", color=GREY, lw=1)
    ax.set_ylim(0, max(np.abs(xv)) + 8)
    ax.set_title("① 湿点 |xv|≤1 (红)=像素 127/128/129; 干点(绿)可安全减幅", fontsize=9.5)
    ...
    ax.text(0, max(counts) + 0.1, "危险带 |xv|≤1", ha="center", color=WARM, fontsize=9)
    ax.set_xlabel("xv (有符号系数 = 像素−128)"); ax.set_ylabel("像素数"); ax.set_xticks(xv_range)
    ax.set_title("② 越靠近 0 越危险: 减幅会撞向 127/128/129", fontsize=9.5)
    fig.suptitle("图 5-1  nsF5 的「湿点/干点」: 先把碰不得的位置划出来", fontsize=11)
    fig.tight_layout(rect=[0, 0, 1, 0.90])
    out("nsf5_wet_dry.png")
...
def fig_solve():
    ...
    axes[0].set_title("① H 矩阵: 湿列(淡红框)不参与, 干列(绿框)可解, 绿粗框=命中列", fontsize=8.5)
    axes[0].set_xlabel("像素位 1~7 (列)", fontsize=8)

    ax = axes[1]
    ax.set_title("② 期望伴随式差 d = s⊕m", fontsize=8.5)
    ...
    ax.set_xticks(range(n)); ax.set_xticklabels(range(1, n + 1), fontsize=7)
    ax.set_ylim(-0.15, 1.4); ax.set_yticks([0, 1]); ax.set_ylabel("是否改动", fontsize=9)
    ax.set_title(f"③ 解: 改 {len(sol)} 个干位置  列{sol}", fontsize=9)
    fig.suptitle(f"图 5-2  湿纸编码: 在干列上解 H·y = d  (本例: h{sol[0]}⊕h{sol[1]} = "
                 f"{list(map(int, H[:,sol[0]-1]))}⊕{list(map(int, H[:,sol[1]-1]))} = {list(map(int, d))} = d  ✔)",
                 fontsize=10)
