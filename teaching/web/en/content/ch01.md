...
![fig-9](assets/img009.png)

Top row above: an **8-bit grayscale image** is split into eight **bit planes** (bit 7 down to bit 0, each labeled with its weight ×128 ... ×1). Bottom row: **accumulate the weighted planes from the most significant down** - with only bit 7 the picture is a fuzzy silhouette; each extra plane makes it sharper; **only when all eight are summed do you get exactly the original image on the left**.

That is the single most important sentence about "stacking":

$$
\text{original} = \sum_{k=0}^{7} \text{bit plane}_k \times 2^{k}
$$

A bit plane is only 0/1; **it must be multiplied by its weight $2^k$ before it can be added to the others.** If you merely OR the eight planes together or weight them by the wrong amount, you do not get the original. This is exactly why the project flips an LSB with `& 0xFE | bit` (clear the lowest bit then set it) rather than adding or subtracting integers.
...
> **Try it |** Open the first block of the interactive lab (`webapp/index.html`, bit-plane lab): pull out bit 0, bit 4 and bit 7 in "Extract plane" mode and compare them; then switch to "Restack (weighted)", turn off bit 0, 1, 2 and watch the image flatten, then turn all 8 back on to see it reconstruct the original exactly.
