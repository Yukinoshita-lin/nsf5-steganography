...
dict(num=8, title="用机器学习做隐写检测", weeks="第 8–14 周", scenes=[
    ...
    S("bullets", "数据与训练：三条纪律", [
        ...
        B("四个候选分类器——逻辑回归、随机森林、梯度提升、XGBoost——比下来 AUC 几乎重叠。结论反直觉但重要：瓶颈不在分类器，在特征。", 2),
    ], ["数据：每照片 1 干净 + 6 含密（1+6）",
        "纪律：预留测试集 / GroupKFold OOF / 校准挑阈值 / 只评一次",
        "~LR、RF、GB、XGB：AUC 几乎重叠 → 瓶颈在特征"]),
    ...
    S("bullets", "v1.4 演进：三次升级，一堂贵课", [
        B("最后看版本演进的三课。SRM 高通滤波：同源数据涨三个点，跨源反而亏——预处理不是万金油。特征 11 维升到 143 维、变体 6 档升 12 档后，AUC 推到 0.81。", 2),
    ...
    ], ["SRM 预处理：同源 +0.03，跨源反亏（不是万金油）",
        "143 维 + 12 档：OOF ≈ 0.81",
        "~教训：真实 JPEG 干净图全判 1.0 → 分布漂移 → 加负样本修复",
        "双版本：143d 稳健（默认）/ 53d 可解释（AUC 更高）"]),
